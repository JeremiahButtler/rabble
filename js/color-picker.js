/**
 * @file
 * Provides UI/UX progressive enhancements on rabble's theme settings by
...
((Drupal, settings, once) => {
  const colorSchemeOptions = settings.rabble.colorSchemes;
  ...
  function setColorScheme({ target }) {
    ...
    if (selectedColorScheme) {
      ...
      document
        .querySelectorAll(`input[data-rabble-custom-color]`)
        .forEach((input) => {
          input.value = input.getAttribute('data-rabble-custom-color');
        });
    }
  }
  ...
  /**
   * Initializes rabble theme-settings color picker.
  ...
  function initColorPicker(textInput) {
    ...
    colorInput.setAttribute(
      'data-rabble-custom-color',
      textInput.getAttribute('data-rabble-custom-color'),
    );
  ...
  /**
   * rabble Color Picker behavior.
   *
   * @type {Drupal~behavior}
   * @prop {Drupal~behaviorAttach} attach
   *   Initializes color picker fields.
   */
  Drupal.behaviors.rabbleColorPicker = {
    attach: () => {
      ...
      const colorTextInputs = once(
        'rabble-color-picker',
        '[data-drupal-selector="rabble-color-picker"] input[type="text"]',
      );
